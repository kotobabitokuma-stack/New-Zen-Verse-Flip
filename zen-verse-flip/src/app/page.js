...
const WORDS = [
  ...
  { id: 9, mainEn: "Keisei Saimin: Enriching the World.", subJp: "経世済民（けいせいさいみん）", noteEn: "The true economy is to govern for the people.", noteJp: "経済とは世を治め民を救うこと。富が巡る豊かな世界を共に作りましょう。" },
  { id: 10, mainEn: "When in doubt, choose with love.", subJp: "迷った時こそ愛の選択", noteEn: "A choice made with love will always lead to fulfillment.", noteJp: "あなたの内側にある「愛」を基準に選んでみてください。" },
  { id: 11, mainEn: "Cherish yourself first.", subJp: "自分を大切に生きること", noteEn: "To protect what you love, you must first value yourself.", noteJp: "あなたが満たされてこそ、大切な人を愛せるようになります。" },
  ...
  { id: 13, mainEn: "Common sense is often a matter of convenience.", subJp: "常識は大体、誰かの都合。", noteEn: "Standards change with time and place.", noteJp: "誰かの都合で決められた「普通」に縛られなくていい。" },
  { id: 14, mainEn: "The direction you are facing is \"forward.\"", subJp: "真っ暗で何も見えなくても", noteEn: "Whichever way you face is the path forward.", noteJp: "あなたが向いた方を「前」だと信じて。あなたなら、大丈夫。" },
  { id: 15, mainEn: "I feel a little embarrassed, but I want to tell you that I love you.", subJp: "ちょっと照れくさいけど", noteEn: "Your love is a gift. Share it.", noteJp: "照れくさくても、大切な人には言葉で伝えましょう。" },
  { id: 16, mainEn: "Take three steps forward, then take two more with vigor", subJp: "三歩すすんで二歩すすむ", noteEn: "Even if you stumble, it's a sign of progress.", noteJp: "自分を信じて、未来へ突き進みましょう。" },
  { id: 17, mainEn: "I'll always give you a push from behind", subJp: "いつでも背中を押してやる", noteEn: "I am always here to support you.", noteJp: "私はいつでも、この言葉を通してあなたの味方です。" },
  ...
  { id: 21, mainEn: "It's okay, the sun is always watching over you!", subJp: "大丈夫いつだってお天道さんが見ているよ", noteEn: "Even if no one notices your hard work, the sun is always watching.", noteJp: "あなた自身が自分の努力を知っている、それだけで十分なのです。" },
  ...
  { id: 23, mainEn: "There's someone tracing your footsteps", subJp: "君の足あとを辿る人がいる", noteEn: "Your footprints will become a guide for those who follow.", noteJp: "道なき道を進むあなたの勇気が、いつか誰かの道しるべになります。" },
  ...
  { id: 25, mainEn: "Letter to the Future", subJp: "未来への手紙", noteEn: "Your life is a blank sheet of paper.", noteJp: "未来はいつだって、あなた次第で描き変えられます。" },
  ...
  { id: 29, mainEn: "Cherished Gems of Our Past Experiences", subJp: "ぜんぶぜんぶ宝もの", noteEn: "Joy, pain, and regrets have shaped you.", noteJp: "喜びも悲しみも、何一つ間違いではありませんでした。" },
...
export default function Home() {
  ...
  const onIncompletePaymentFound = async (payment) => {
    console.log("未完了決済を発見:", payment.identifier);
    try {
    ...
    } catch (err) { console.error("お掃除失敗:", err); }
  };
  ...
  const nextCard = () => { if (user) { setIndex((prev) => (prev + 1) % WORDS.length); setShowNote(false); } };
  const prevCard = () => { if (user) { setIndex((prev) => (prev - 1 + WORDS.length) % WORDS.length); setShowNote(false); } };
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          <motion.main key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col items-center bg-white">
            ...
            {/* 1. コンテンツエリア */}
            <div className="flex-1 w-full max-w-sm flex items-center justify-center px-6 relative"
                 onClick={() => user && setShowNote(!showNote)}>
              {index === 0 ? (
                <img src={WORDS[0].image} className="w-full h-auto shadow-2xl rounded-lg" alt="Cover" />
              ) : (
                <div className="text-center p-8 border-2 border-gray-100 rounded-3xl bg-white shadow-xl min-h-[300px] flex flex-col justify-center">
                  <h2 className="text-xl font-bold mb-4">{showNote ? WORDS[index].noteEn : WORDS[index].mainEn}</h2>
                  <p className="text-sm text-gray-500">{showNote ? WORDS[index].noteJp : WORDS[index].subJp}</p>
                </div>
              )}
            </div>
            ...
            <div className="w-full max-w-[280px] py-4">
              {!user ? (
                ...
                <div className="text-center space-y-2">
                  <div className="py-2 px-4 bg-gray-50 rounded-full border border-gray-200 inline-block">
                    <p className="text-xs font-bold text-gray-700">ようこそ、{user.username} くん！</p>
                  </div>
                  <p className="text-[10px] text-gray-400">画面タップで詳細を表示 / 左右スワイプで切り替え</p>
                </div>
              )}
            </div>
            ...
            <footer className="w-full text-center pb-8 shrink-0">
              <div className="flex justify-center gap-10 mb-4">
                {user && (
                  <>
                    <button onClick={prevCard} className="p-2 text-gray-400">←</button>
                    <button onClick={nextCard} className="p-2 text-gray-400">→</button>
                  </>
                )}
              </div>
