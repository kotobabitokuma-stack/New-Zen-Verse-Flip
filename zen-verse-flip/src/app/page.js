...
const WORDS = [
  ...
  { id: 16, mainEn: "Take three steps forward, then take two more with vigor", subJp: "三歩すすんで二歩すすむ", noteEn: "Even if you stumble, it's a sign of progress. Only those who keep moving get to enjoy the beautiful scenery ahead.", noteJp: "自分を信じて、未来へ突き進みましょう。転ぶのは前に進しようとしている証拠です。歩き続けた人にしか見えない景色を、楽しんでください。" },
        ...
        {isLoading ? (
          ...
          <motion.main key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col items-center bg-white">
            ...
            <div className="flex-1 w-full max-w-sm flex items-center justify-center px-6">
               ...
               {index !== 0 && (
                  <div className="text-center p-6 border rounded-lg shadow-sm bg-white" onClick={() => setShowNote(!showNote)}>
                    <h2 className="text-xl font-bold">{showNote ? WORDS[index].noteEn : WORDS[index].mainEn}</h2>
                    <p className="mt-4 text-sm text-gray-600">{showNote ? WORDS[index].noteJp : WORDS[index].subJp}</p>
                  </div>
               )}
            </div>
            ...
            <div className="w-full max-w-[280px] py-4">
              {!user ? (
                ...
                <div className="text-center space-y-4">
                  ...
                  <p className="text-[12px] text-gray-400">スワイプして言葉を受け取ってね</p>
                </div>
              )}
            </div>
            ...
            <footer className="w-full text-center pb-8 shrink-0">
              <div className="flex justify-center space-x-8 mb-4">
                  {user && index !== 0 && (
                    <>
                      <button onClick={prevCard} className="text-gray-400">← Prev</button>
                      <button onClick={nextCard} className="text-gray-400">Next →</button>
                    </>
                  )}
                  {user && index === 0 && (
                      <button onClick={nextCard} className="py-2 px-6 bg-black text-white rounded-full text-sm">開く</button>
                  )}
              </div>
